...
namespace renderer
{
	...
	static void LoadCustomFont()
	{
		...
		for (int i = 0; i < _fontsCount; i++)
		{
			...
			auto newFont = io.Fonts->AddFontFromMemoryTTF(
				_customFontData.data, _customFontData.size,
				static_cast<float>((i + 1) * _fontSizeStep));
		...
		ImGuiIO& io = ImGui::GetIO();
		ImFont* font = io.Fonts->AddFontFromFileTTF(
			"c:\\Windows\\Fonts\\simhei.ttf", 20.0f, NULL,
			io.Fonts->GetGlyphRangesChineseFull());
		IM_ASSERT(font != NULL);
		_fonts[0] = font;
